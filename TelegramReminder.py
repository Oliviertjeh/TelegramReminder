...
def parse_dt(text: str):
    ...
    try:
        ...
        print(f"[DEBUG parse_dt] Couldn’t parse {text!r}")
        return None
    ...
    print(f"[DEBUG parse_dt] parsed {dt_local.strftime('%Y-%m-%d %H:%M %Z')} -> {dt_utc.strftime('%Y-%m-%d %H:%M %Z')}")
    return dt_utc
...
async def schedule_reminder(ev, when, caption, media_path=None):
    ...
    if media_path:
        await client.send_file(ev.chat_id, media_path, caption=text, schedule=when, parse_mode="md")
    else:
        await client.send_message(ev.chat_id, text, schedule=when, parse_mode="md")
    print(f"[DEBUG scheduled] chat={ev.chat_id} when={when.isoformat()}")
...
# ─── HELP COMMAND ─────────────────────────────────────────────────────────────
@client.on(events.NewMessage(pattern=re.compile(r"^/help reminder$", re.IGNORECASE)))
